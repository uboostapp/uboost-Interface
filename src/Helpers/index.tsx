...
export function shortenAddress(address: string, chars = 4): string | undefined {
  if (address) {
...
export const getTransactionHashUrl = (activeNetWork, hash) => {
  ...
  let maticURL = `polygonscan.com/tx/${hash}`
  switch (activeNetWork) {
    ...
    case 'Mumbai Testnet':
      return `https://mumbai.${maticURL}`
    default:
      return `https://${activeNetWork}.${baseURL}`
  }
}
...
export const checkNet = (net: number) => {
  switch (net) {
    case 1:
      return 'Mainnet'
    case 42:
...
export const checkNetwork = (net: number) => {
  switch (net) {
    case 56:
      return 'Binance'
    case 97:
