...
export default function WalletProvider(props) {
  ...
  const { allCoinList, allCoinList2, tokenImportHandler, token2ImportHandler } =
    useCustomToken();
  ...
  useEffect(() => {
    if (connectedWallet !== null && connectedWallet !== undefined)
      Cookies.set("wallet", connectedWallet);
  }, [connectedWallet]);
    ...
    const getToken = async () => {
      if (qpTokens?.main_token) {
        let token = await allCoinList.find((item) => {
          return (
            item.address.toLowerCase() === qpTokens.main_token.toLowerCase()
          );
        });
        if (token) {
          setToken1(token);
      ...
      if (qpTokens?.base_token) {
        let token = allCoinList2.find(
          (item) =>
            item.address.toLowerCase() === qpTokens.base_token.toLowerCase()
        );
        if (token) setToken2(token);
      }
      if (qpTokens?.dex) {
        let dex = dexList.find(
          (item) => item.name.toLowerCase() === qpTokens.dex.toLowerCase()
        );
        if (dex) setDex(dex);
      }
      if (qpTokens?.network) {
        let network = await chainList.find(
          (item) => item.name.toLowerCase() === qpTokens.network.toLowerCase()
        );
  ...
  const handleSearch2Http = (network: number, address: string) => {
    ...
    axios
      .get(
        `https://api.covalenthq.com/v1/${network}/tokens/${address}/nft_metadata/111/${qs}`
      )
      .then((res: AxiosResponse) => {
        if (res?.data?.data) {
  ...
  const handleChainChange = (id: number) => {
    let _chain: Chain | undefined = chainList.find(
      (list: Chain) => list.id === id
    );
    if (_chain) {
